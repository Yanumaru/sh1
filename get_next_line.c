...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2014/01/26 20:37:29 by ykichena          #+#    #+#             */
/*   Updated: 2014/11/16 20:43:14 by ykichena         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_realloc_gnl(char **line, char *buf, int size, int *len)
{
	char	*temp;
...
static int	ft_read_buf(char **line, t_info *cur, int *len)
{
	...
	{
		if (ft_realloc_gnl(line, cur->start, cur->offset, len))
			return (-1);
		cur->offset = 0;
		cur->start = NULL;
	}
	...
	{
		if (ft_realloc_gnl(line, cur->start, n - cur->start, len))
			return (-1);
